...
/**
 * @author lingwh
 * @desc 演示BlockingQueue机制的使用场景：应用启动需要等待多个关键服务初始化完成
...
public class BlockingQueueDemoTest {
    ...
    // 创建容量为TOTAL_SERVICES的阻塞队列
    private static final BlockingQueue<String> initializationQueue = 
        new ArrayBlockingQueue<>(TOTAL_SERVICES);
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            // 从队列中取出TOTAL_SERVICES个初始化完成信号
            for (int i = 0; i < TOTAL_SERVICES; i++) {
                initializationQueue.take();
            }
    ...
    static class ServiceInitializer implements Runnable {
        ...
        public void run() {
            try {
                ...
                // 向队列中放入初始化完成信号
                initializationQueue.offer(serviceName);
            } catch (InterruptedException e) {
                Thread.currentThread().interrupt();
            }
        }
