...
//定义语法
  final public String parseSql() throws ParseException {
    trace_call("parseSql");
    try {
Token token ;
...
System.out.println(token.image);
      token = jj_consume_token(TABLE);
System.out.println(token.image);
      token = jj_consume_token(VARIABLE_NAME);
System.out.println(token.image);
      token = jj_consume_token(RIGHT_PARENTHESES);
      label_1:
      while (true) {
        ...
        token = jj_consume_token(VARIABLE_NAME);
System.out.println(token.image);
        token = AttrituabeType();
System.out.println(token.image);
        switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
        case COMMENT:{
          token = jj_consume_token(COMMENT);
System.out.println(token.image);
          jj_consume_token(SINGLE_QUOTATION_MARK);
          token = jj_consume_token(COMMENT_CONTENT);
System.out.println(token.image);
          jj_consume_token(SINGLE_QUOTATION_MARK);
          break;
          }
 ...
 * 获取属�?�的类型
 */
  final public Token AttrituabeType() throws ParseException {
    trace_call("AttrituabeType");
    try {
    Token token;
      switch ((jj_ntk==-1)?jj_ntk_f():jj_ntk) {
      case INT:{
  ...
  static {
	...
	private static void jj_la1_init_0() {
	   jj_la1_0 = new int[] {0x1000,0x80,0x10,0x80,0x300,};
	}
